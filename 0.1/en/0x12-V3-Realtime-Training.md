...
| 3.3 | Ensure the LLM is trained on adversarial input to recognize the most widespread attacks, such as prompt injection, jailbreaking, context injection, etc. |      | ✓ | ✓ |
| 3.4 | Make sure that Reinforcement Learning from Human Feedback (RLHF) is integrated into the LLM fine-tuning process. |      | ✓ | ✓ |
| 3.5 | Ensure that instructions are separated from data with a unique character string. The LLM should be fine-tuned to only identify this string as a separator between instructions and data. |      | ✓ | ✓ |
