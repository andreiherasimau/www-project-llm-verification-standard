...
| 2.2.2 | Ensure that foundational models are fine-tuned to limit irrelevant data points which may lead to poor model performance. |      | ✓ | ✓ |
| 2.2.3 | Ensure that the LLM is regularly fine-tuned to prevent the most recent vulnerabilities. |      | ✓ | ✓ |
| 2.2.4 | Ensure that the LLM is fine-tuned to only output verified and truthful information. |      | ✓ | ✓ |
...
| 2.4.4 | Ensure security tooling to detect LLM vulnerabilities such as injection attacks, jailbreak attempts and other abuse are integrated into the ML models lifecycle. |      | ✓ | ✓ |
